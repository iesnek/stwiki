...
<?php if(is_tax('equip',array('buki','bushi','yari','yumi','miko','ninja','yokenshi'))): ?>
  <div class="l-section dbMenu dbMenu_buki">
    <ul class="arrow">
...
<?php elseif(is_tax('equip',array('bogu','head','mask','neck','body','waist','shoulder','arm','leg'))): ?>
  ...
  </div><!-- .l-section dbMenu dbMenu_bogu -->
<?php elseif(is_tax('equip',array('irui','inner','outer','shoes','front','option'))): ?>
  <div class="l-section dbMenu dbMenu_irui">
    <ul class="arrow">
...
<?php elseif(is_tax('equip',array('fuda','bushi-fuda','yari-fuda','yumi-fuda','miko-fuda','ninja-fuda','yokenshi-fuda'))): ?>
  ...
  </div><!-- .l-section dbMenu dbMenu_fuda -->
<?php elseif(is_tax('equip',array('make','hair','eye','mouth','face','skin','background'))): ?>
  <div class="l-section dbMenu dbMenu_make">
    <ul class="arrow">
...
<?php endif; endif; ?>
  ...
  </div><!-- .l-section dbMenu dbMenu_make -->
<?php endif; ?>
...
<div class="article">
  <h1>「<?php the_search_query(); ?>」の検索結果 : 
    <?php if($wp_query -> found_posts <= 1){ //ポスト数
...
<header class="archive_headline">
  <ul>
    ...
    <li>
      ...
      if(strstr($url,'cost')==true && strstr($url,'ASC')==true){ echo '<a href="' . add_query_arg( array('meta_key' => 'cost', 'orderby' => 'meta_value_num', 'order' => 'DESC'), get_pagenum_link(1) ) . '">ｺｽﾄ<br>▲</a>'; }
      elseif(strstr($url,'cost')==true && strstr($url,'DESC')==true){ echo '<a href="' . add_query_arg( array('meta_key' => 'cost', 'orderby' => 'meta_value_num', 'order' => 'ASC'), get_pagenum_link(1) ) . '">ｺｽﾄ<br>▼</a>'; }
      else{ echo '<a href="' . add_query_arg( array('meta_key' => 'cost', 'orderby' => 'meta_value_num', 'order' => 'DESC'), get_pagenum_link(1) ) . '">ｺｽﾄ<br>▽</a>'; }
      ?>
    </li>
    <li>
      <?php $url = $_SERVER['REQUEST_URI'];
